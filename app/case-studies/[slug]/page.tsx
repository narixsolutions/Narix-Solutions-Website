    ...
    <main className="min-h-screen bg-background">
      <section className="pt-28 pb-12 px-4 sm:px-6 lg:px-8 border-b border-border/60 bg-card/40">
        <div className="max-w-6xl mx-auto">
          <Link
            href="/case-studies"
            className="text-sm font-semibold text-foreground/70 hover:text-accent inline-flex items-center gap-2 mb-8 transition-colors"
          >
            <ArrowLeft size={16} />
            All case studies
          </Link>

          <div className="inline-flex px-4 py-1.5 rounded-full border border-accent/25 bg-accent/10 text-accent text-sm font-semibold mb-6">
            {study.category}
          </div>
          ...
          <div className="mt-12 grid gap-6 lg:grid-cols-[1.1fr_0.9fr] items-start">
            <div className="relative aspect-[16/10] rounded-2xl overflow-hidden border border-border bg-muted/30">
              <Image
                src={study.heroImage}
          ...
          <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
            {study.gallery.map((src, idx) => (
              <div key={src} className="relative aspect-[4/3] rounded-xl overflow-hidden border border-border bg-muted/20">
                <Image
                  src={src}
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8 border-t border-border bg-card/50">
        <div className="max-w-3xl mx-auto text-center space-y-6">
          <h2 className="text-3xl font-bold text-foreground">Planning something similar?</h2>
          <p className="text-foreground/70">
            We&apos;ll assemble a roadmap, staffing model, and risk register tailored to your constraints.
          </p>
          <Link
            href="/contact"
