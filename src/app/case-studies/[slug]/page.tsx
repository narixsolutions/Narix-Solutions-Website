...
import { FadeInView, AnimateStagger, AnimateItem } from '@/components/ui/animate';
    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="pt-16 pb-6 px-4 sm:px-6 lg:px-8 border-b border-border/60 bg-card/40">
        <div className="max-w-6xl mx-auto">
          {/* Back nav + category */}
          <FadeInView className="mb-10 flex flex-wrap items-center gap-4 sm:gap-5">
            <Link
              href="/case-studies"
          ...
          <FadeInView>
            ...
            {study.heroDescription && (
              <p className="text-base text-foreground/65 max-w-4xl mt-4">{study.heroDescription}</p>
            )}
          </FadeInView>

          {/* Tags */}
          {study.tags && study.tags.length > 0 && (
            <FadeInView className="mt-6 flex flex-wrap gap-2.5">
              {study.tags.map((tag) => (
          ...
          {/* Hero image + metrics */}
          <FadeInView className="mt-12 grid gap-6 lg:grid-cols-[1.1fr_0.9fr] items-start">
          ...
          {/* Gallery strip */}
          <AnimateStagger className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-4">
            {study.gallery.map((src, idx) => (
              <AnimateItem key={`${src}-${idx}`}>
                <div className="relative aspect-[4/3] rounded-xl overflow-hidden border border-border bg-muted/20">
                  <Image
                    src={src}
            ...
            <div className="mt-14 space-y-8">
              {study.sections.map((section, sectionIdx) => (
                <FadeInView key={section.title}>
                  <article className="rounded-2xl border border-border bg-card/80 p-5 md:p-7 shadow-sm hover:border-accent/30 transition-colors">
                        ...
                        {section.techStack && section.techStack.length > 0 && (
                          <div className="flex flex-wrap gap-2">
                            {section.techStack.map((tech) => (
                              <span key={tech} className="rounded-lg border border-border bg-secondary/60 px-3 py-1 text-xs font-medium text-foreground/85">
                                {tech}
                              </span>
                            ))}
                          </div>
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto space-y-14">
            ...
            <article className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                {study.technologies.map((tech) => (
                  <span key={tech} className="rounded-lg border border-border bg-secondary/60 px-3 py-1.5 text-sm font-medium text-foreground/85">
                    {tech}
                  </span>
                ))}
              </div>
          ...
          <FadeInView className="grid sm:grid-cols-2 gap-8 pt-8 border-t border-border">
            <div>
      ...
      <section className="py-14 px-4 sm:px-6 lg:px-8 border-t border-border bg-card/50">
        <FadeInView className="max-w-3xl mx-auto text-center space-y-6">
