    ...
    <section className="py-16 px-4 sm:px-6 lg:px-8 bg-card/50 border-y border-border">
      <div className="max-w-7xl mx-auto">
        ...
        <motion.div
          ...
          className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5 xl:gap-6 mb-8"
        >
            ...
            <motion.div key={project.slug} variants={staggerItem}>
              <Link href={`/case-studies/${project.slug}`} className="group block">
                <div className="relative h-72 rounded-xl overflow-hidden bg-card border border-border hover:border-accent/40 transition-all shadow-sm">
                  ...
                  <div className="absolute top-4 right-4 rounded-full bg-background/85 border border-border px-3 py-1 text-xs font-semibold text-foreground">
                    {project.category}
                  </div>
